//
//  IGGSideMenuContainerViewController.m
//  IgSideMenu
//
...
NSString * const IGGSideMenuStateNotificationEvent = @"IGGSideMenuStateNotificationEvent";

typedef enum {
    IGGSideMenuPanDirectionNone,
    IGGSideMenuPanDirectionLeft,
    IGGSideMenuPanDirectionRight
} IGGSideMenuPanDirection;

@interface IGGSideMenuContainerViewController ()
@property (nonatomic, strong) UIView *menuContainerView;

@property (nonatomic, assign) CGPoint panGestureOrigin;
@property (nonatomic, assign) CGFloat panGestureVelocity;
@property (nonatomic, assign) IGGSideMenuPanDirection panDirection;
@end
...
@implementation IGGSideMenuContainerViewController
...
+ (IGGSideMenuContainerViewController *)containerWithCenterViewController:(id)centerViewController
    ...
    IGGSideMenuContainerViewController *controller = [IGGSideMenuContainerViewController new];
...
- (void)setDefaultSettings {
    ...
    self.menuState = IGGSideMenuStateClosed;
    ...
    self.panMode = IGGSideMenuPanModeDefault;
}
...
- (void)toggleLeftSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == IGGSideMenuStateLeftMenuOpen) {
        [self setMenuState:IGGSideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:IGGSideMenuStateLeftMenuOpen completion:completion];
    }
}

- (void)toggleRightSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == IGGSideMenuStateRightMenuOpen) {
        [self setMenuState:IGGSideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:IGGSideMenuStateRightMenuOpen completion:completion];
    }
}
...
- (void)setMenuState:(IGGSideMenuState)menuState {
...
- (void)setMenuState:(IGGSideMenuState)menuState completion:(void (^)(void))completion {
    void (^innerCompletion)() = ^ {
        ...
        IGGSideMenuStateEvent eventType = (_menuState == IGGSideMenuStateClosed) ? IGGSideMenuStateEventMenuDidClose : IGGSideMenuStateEventMenuDidOpen;
    ...
    switch (menuState) {
        case IGGSideMenuStateClosed: {
            [self sendStateEventNotification:IGGSideMenuStateEventMenuWillClose];
        ...
        case IGGSideMenuStateLeftMenuOpen:
            if(!self.leftMenuViewController) return;
            [self sendStateEventNotification:IGGSideMenuStateEventMenuWillOpen];
            [self leftMenuWillShow];
            [self openLeftSideMenuCompletion:innerCompletion];
            break;
        case IGGSideMenuStateRightMenuOpen:
            if(!self.rightMenuViewController) return;
            [self sendStateEventNotification:IGGSideMenuStateEventMenuWillOpen];
...
- (void)sendStateEventNotification:(IGGSideMenuStateEvent)event {
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:event]
                                                         forKey:@"eventType"];
    [[NSNotificationCenter defaultCenter] postNotificationName:IGGSideMenuStateNotificationEvent
                                                        object:self
                                                      userInfo:userInfo];
}
...
#pragma mark -
#pragma mark - IGGSideMenuPanMode

- (BOOL) centerViewControllerPanEnabled {
    return ((self.panMode & IGGSideMenuPanModeCenterViewController) == IGGSideMenuPanModeCenterViewController);
}

- (BOOL) sideMenuPanEnabled {
    return ((self.panMode & IGGSideMenuPanModeSideMenu) == IGGSideMenuPanModeSideMenu);
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]] &&
       self.menuState != IGGSideMenuStateClosed) return YES;
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
	...
	if(recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.panDirection = IGGSideMenuPanDirectionNone;
	}
    
    if(self.panDirection == IGGSideMenuPanDirectionNone) {
        CGPoint translatedPoint = [recognizer translationInView:view];
        if(translatedPoint.x > 0) {
            self.panDirection = IGGSideMenuPanDirectionRight;
            if(self.leftMenuViewController && self.menuState == IGGSideMenuStateClosed) {
                [self leftMenuWillShow];
            }
        }
        else if(translatedPoint.x < 0) {
            self.panDirection = IGGSideMenuPanDirectionLeft;
            if(self.rightMenuViewController && self.menuState == IGGSideMenuStateClosed) {
                [self rightMenuWillShow];
            }
        }
    }
    
    if((self.menuState == IGGSideMenuStateRightMenuOpen && self.panDirection == IGGSideMenuPanDirectionLeft)
       || (self.menuState == IGGSideMenuStateLeftMenuOpen && self.panDirection == IGGSideMenuPanDirectionRight)) {
        self.panDirection = IGGSideMenuPanDirectionNone;
        return;
    }
    
    if(self.panDirection == IGGSideMenuPanDirectionLeft) {
        [self handleLeftPan:recognizer];
    } else if(self.panDirection == IGGSideMenuPanDirectionRight) {
        [self handleRightPan:recognizer];
    }
}

- (void) handleRightPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.leftMenuViewController && self.menuState == IGGSideMenuStateClosed) return;
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == IGGSideMenuStateClosed) {
            ...
            if(hideMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:IGGSideMenuStateClosed];
            } else {
                self.panGestureVelocity = 0;
...
- (void) handleLeftPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.rightMenuViewController && self.menuState == IGGSideMenuStateClosed) return;
	...
	if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == IGGSideMenuStateClosed) {
            ...
            if(showMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:IGGSideMenuStateRightMenuOpen];
            } else {
                self.panGestureVelocity = 0;
                [self setCenterViewControllerOffset:0 animated:YES completion:nil];
            }
...
- (void)setUserInteractionStateForCenterViewController {
    ...
    if([self.centerViewController respondsToSelector:@selector(viewControllers)]) {
        ...
        for(UIViewController* viewController in viewControllers) {
            viewController.view.userInteractionEnabled = (self.menuState == IGGSideMenuStateClosed);
        }
    }
}
